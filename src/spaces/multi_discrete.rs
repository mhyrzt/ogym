...
impl MultiDiscrete {
    ...
    /// Maps a multi-discrete vector to a single u32 value
    /// Uses mixed-radix representation where each dimension acts as a different base
    pub fn to_u32(&self, values: &[u32]) -> Result<u32, Error> {
        ...
        for i in (0..values.len()).rev() {
            result = result.checked_add(
                values[i].checked_mul(multiplier)
                    .ok_or(Error::InvalidBounds)?
            ).ok_or(Error::InvalidBounds)?;
            
            multiplier = multiplier.checked_mul(self.nvec[i])
                .ok_or(Error::InvalidBounds)?;
        }
    ...
    /// Maps a u32 value back to a multi-discrete vector
    /// Inverse of multi_discrete_to_u32
