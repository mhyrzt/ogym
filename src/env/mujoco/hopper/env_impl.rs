...
use super::{env::MujocoHopperEnv, config::HopperConfig};
...
impl Environment for MujocoHopperEnv {
    ...
    fn step(
        ...
        info.insert("z_distance_from_origin".to_string(), 
...
impl MujocoHopperEnv {
    ...
    fn _get_rew(&self, x_velocity: f64, action: &DVector<f64>) -> Result<(f64, std::collections::HashMap<String, f64>), Error> {
    ...
    fn is_healthy(&self) -> Result<bool, Error> {
        ...
        let healthy_state = state_without_pos.iter().all(|&x| x > min_state && x < max_state);
