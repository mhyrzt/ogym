...
use super::{env::MujocoHumanoidStandupEnv, config::HumanoidStandupConfig};
...
impl Environment for MujocoHumanoidStandupEnv {
    ...
    fn step(
        ...
        info.insert("tendon_length".to_string(), self.env.ten_length().iter().sum()); // Sum as a placeholder since it's an array
        info.insert("tendon_velocity".to_string(), self.env.ten_velocity().iter().sum()); // Sum as a placeholder since it's an array
...
impl MujocoHumanoidStandupEnv {
    ...
    fn _get_obs(&self) -> Result<DVector<f64>, Error> {
        ...
        let com_inertia = if self.config.include_cinert_in_observation {
            ...
            for body_cinert in cinert.iter().skip(1) { // Skip first body
        ...
        let com_velocity = if self.config.include_cvel_in_observation {
            ...
            for body_cvel in cvel.iter().skip(1) { // Skip first body
        ...
        let external_contact_forces = if self.config.include_cfrc_ext_in_observation {
            ...
            for body_cfrc in cfrc_ext.iter().skip(1) { // Skip first body
    ...
    fn _calculate_reward(&self, action: &DVector<f64>) -> Result<(f64, std::collections::HashMap<String, f64>), Error> {
    ...
    fn _impact_cost(&self) -> Result<f64, Error> {
        ...
        let clamped_cost = impact_cost.clamp(self.config.impact_cost_range.0, self.config.impact_cost_range.1);
